...
function isValidItem(body: any): { ok: boolean; error?: string } {
  ...
  if (typeof body.project !== 'string' || !body.project) return { ok: false, error: 'project must be non-empty string' };
  if (typeof body.timestamp !== 'number' && typeof body.timestamp !== 'string') return { ok: false, error: 'timestamp must be number or ISO string' };
  return { ok: true };
}
...
// When project is provided, remove that project's directory. Otherwise, clear all data under DATA_DIR.
app.delete('/api/data', (req: Request, res: Response) => {
  const project = (req.query.project as string) || undefined;
  try {
    if (project) {
      const projectDir = path.join(DATA_DIR, sanitize(project));
      if (!fs.existsSync(projectDir)) return res.json({ ok: true, cleared: 'project', project, removed: 0 });
...
function readProjectData(project: string, filters: ReadFilters = {}) {
  ...
  const top = fs.readdirSync(projectDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
  ...
  function looksLikeSession(topName: string): boolean {
    try {
      ...
      const tagids = fs.readdirSync(sessDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
  ...
  for (const tag of oldTop) {
    ...
    for (const desc of descs) {
      ...
      const sessList = fs.readdirSync(descDir, { withFileTypes: true }).filter((d) => d.isDirectory()).map((d) => d.name);
...
app.get('/api/data', (req: Request, res: Response) => {
  ...
  return res.json({ project, nest: 'session>tagid>description', filters: { session: qSession, tagid: qTag }, data });
});
